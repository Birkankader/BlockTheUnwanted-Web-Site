...
function initializeThemeAndLanguage() {
  ...
  const fromOptions =
    urlParams.get("from") === "options" ||
    window.location.href.includes("options_page");

  console.log("[Website] Initialization params:", {
    urlLang,
    urlTheme,
    fromExtension,
    fromOptions,
  ...
  const isOptionsPage =
    !urlParams.toString() && window.location.href.includes("extension-website");
  ...
  function initializeFallback() {
    ...
    if (urlLang && urlTheme) {
      ...
      console.log("[Website] Using URL parameters:", {
        currentLang,
        currentTheme,
      });
    } else {
      ...
      console.log("[Website] Using localStorage:", {
        currentLang,
        currentTheme,
      });
    }
    ...
    try {
      ...
      if (typeof chrome !== "undefined" && chrome.storage) {
        ...
        chrome.storage.local.get(
          ["twitter-filter-language", "twitter-filter-theme"],
          (result) => {
            ...
            if (urlLang && urlTheme) {
              ...
              currentLang = urlLang;
              currentTheme = urlTheme;
              console.log("[Website] Using popup parameters:", {
                currentLang,
                currentTheme,
              });
            } else {
              ...
              console.log("[Website] Using extension storage:", {
                currentLang,
                currentTheme,
              });
            }
      ...
      console.log(
        "[Website] Could not access extension storage, using fallback:",
        error
      );
    }
  }
...
function initializeEmailJS() {
  ...
  if (typeof emailjs !== "undefined" && isEmailConfigComplete()) {
    ...
    console.warn(
      "[Website] EmailJS not available or environment variables not set. Please check your .env file or Netlify environment variables."
    );
    console.warn(
      "[Website] Expected variables: VITE_EMAILJS_SERVICE_ID, VITE_EMAILJS_TEMPLATE_ID, VITE_EMAILJS_PUBLIC_KEY"
    );
    return false;
  }
}
...
function applyTranslations(lang) {
  ...
  const placeholderElements = document.querySelectorAll(
    "[data-en-placeholder][data-tr-placeholder]"
  );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (installBtn) {
    installBtn.addEventListener("click", function (e) {
      ...
      const chromeWebStoreUrl =
        "https://chromewebstore.google.com/detail/block-the-unwanted/pchajhojhlhfaealmbncjjkabmemaefb";
  ...
  if (bugReportForm) {
    ...
    function updateFormPlaceholders() {
      const placeholderElements = bugReportForm.querySelectorAll(
        "[data-en-placeholder][data-tr-placeholder]"
      );
      placeholderElements.forEach((element) => {
      ...
      const selectElements = bugReportForm.querySelectorAll(
        "select option[data-en][data-tr]"
      );
    ...
    function populateBrowserInfo() {
      ...
      if (browserField) {
        ...
        if (userAgent.includes("Chrome")) {
          ...
          browserInfo = `Chrome ${
            chromeVersion ? chromeVersion[1] : "Unknown Version"
          }`;
        } else if (userAgent.includes("Firefox")) {
          const firefoxVersion = userAgent.match(/Firefox\/([0-9.]+)/);
          browserInfo = `Firefox ${
            firefoxVersion ? firefoxVersion[1] : "Unknown Version"
          }`;
        } else if (userAgent.includes("Safari")) {
          const safariVersion = userAgent.match(/Version\/([0-9.]+)/);
          browserInfo = `Safari ${
            safariVersion ? safariVersion[1] : "Unknown Version"
          }`;
        } else {
...
document.addEventListener("DOMContentLoaded", function () {
  const animatedElements = document.querySelectorAll(
    ".feature-card, .step, .faq-item, .pricing-card"
  );
